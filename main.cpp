...
void creatingCactusArray(std::vector<Object> &array, int fromWhichIndex, int toWhatIndex) {
    for (int i = fromWhichIndex; i <= toWhatIndex; i++) {
        if (i == 0) {
            ...
            if (randomCactus == 0) {
                array[i].setObject("Files/Img/Cactus_1.png", 40, 88, 1000, 420, 5);
            } else if (randomCactus == 1) {
                array[i].setObject("Files/Img/Cactus_2.png", 49, 93, 1000, 415, 5);
            } else if (randomCactus == 2) {
                array[i].setObject("Files/Img/Cactus_3.png", 44, 104, 1000, 405, 5);
            }
        } else {
            int randomCactus = randomCactusIndex(3);
            ...
            if (randomCactus == 0) {
                ...
                array[i].setObject("Files/Img/Cactus_1.png", 40, 88, newX, 420, 5);
            } else if (randomCactus == 1) {
                array[i].setObject("Files/Img/Cactus_2.png", 49, 93, newX, 415, 5);
            } else if (randomCactus == 2) {
                array[i].setObject("Files/Img/Cactus_3.png", 44, 104, newX, 405, 5);
            }
        }
    }
}
...
int runGame() {
    // Creating dino
    Dino dino_1("Files/Img/Dino_1.png", 80, 140, WINDOWWIGHT / 6, WINDOWHEIGHT - 140 - 100);
    Dino dino_2("Files/Img/Dino_2.png", 80, 140, WINDOWWIGHT / 6, WINDOWHEIGHT - 140 - 100);
    Dino dino_3("Files/Img/Dino_3.png", 80, 140, WINDOWWIGHT / 6, WINDOWHEIGHT - 140 - 100);
    Dino dino_Jump("Files/Img/Dino_Jump.png", 80, 140, WINDOWWIGHT / 6, WINDOWHEIGHT - 140 - 100);
    Dino dino_Rip("Files/Img/Dino_RIP.png", 80, 140, WINDOWWIGHT / 6, WINDOWHEIGHT - 140 - 100);
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if ((event.type == sf::Event::KeyPressed) && (event.key.code == sf::Keyboard::Space)) {
                dino_1.make_jump_flag = true;  // Jump
            }
        }

        // Jump
        if (dino_1.make_jump_flag) {
            dino_1.jump();
            dino_2.jump();
            dino_3.jump();
            dino_Jump.jump();
        ...
        // Setting jumping texture
        if (dino_1.make_jump_flag) {
            window.draw(dino_Jump.dinoSprite);
        } else {
            // Setting running texture
            madeAnimation++;
            if (madeAnimation < 20) {
                window.draw(dino_1.dinoSprite);
            } else if (madeAnimation < 40) {
                window.draw(dino_2.dinoSprite);
            } else if (madeAnimation < 60) {
                window.draw(dino_3.dinoSprite);
            } else if (madeAnimation == 60) {
                window.draw(dino_1.dinoSprite);
                madeAnimation = 0;
            }
        ...
        varForFixBug++;
        if (checkingCactusAndDinoCollision(arrayWithCactuses, dino_1)) {
            if (varForFixBug < 10) {
                return 1;
            } else {
                window.draw(dino_Rip.dinoSprite);
                if (gameOver() == 0) {
                    return 1;
                } else {
                    window.close();
        ...
        if (score > 100) {
            for (int i = 0; i < 10; i++) {
                arrayWithCactuses[i].objectMoveSpeed++;
            }
            score = 0;
        }
